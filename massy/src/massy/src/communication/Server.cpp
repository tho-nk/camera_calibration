...
Server::Server(const std::string& address, const std::string& port) : address_(address), port_(port), ioContext_(), acceptor_(ioContext_), signals_(ioContext_, SIGINT), connectionManager_()
{
...
{
   acceptor_.async_accept([this](boost::system::error_code errorCode, boost::asio::ip::tcp::socket socket) {
      // signal check
