...
namespace massy::communication {
Connection::Connection(boost::asio::ip::tcp::socket&& socket, boost::asio::io_context& ioContext, ConnectionManager& connectionManager)
    : socket_(std::forward<boost::asio::ip::tcp::socket>(socket)), epochTimer_(ioContext), connectionManager_(connectionManager)
{
...
{
   boost::asio::async_read(socket_, request_, boost::asio::transfer_at_least(1), std::bind(&Connection::onRead, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
void Connection::onRead(boost::system::error_code errorCode, size_t transferredBytes)
{
   if (errorCode) {
      ...
      std::cout << "message from client " << this << " := " << &request_ << std::endl;
      ...
      boost::asio::async_write(socket_, boost::asio::buffer("hey\n"), [self](boost::system::error_code errorCode, size_t transferredBytes) {
         ...
         std::cout << "send hey to client " << self << std::endl;
      });
   }
   doRead();
}
...
void Connection::doWrite()
{
   ...
   std::cout << "send " << ms << " to client " << this << std::endl;
   boost::asio::async_write(socket_, boost::asio::buffer(std::to_string(ms) + "\n"), std::bind(&Connection::onWrite, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
void Connection::sendBye()
{
   ...
   boost::asio::async_write(socket_, boost::asio::buffer("bye\n"), [self](boost::system::error_code errorCode, size_t transferredBytes) {
      ...
      std::cout << "send bye to client " << self << std::endl;
   });
}
...
{
   std::cout << "ConnectionManager::start connections_.size() = " << connections_.size() << std::endl;
...
{
   std::cout << "ConnectionManager::stop" << std::endl;
...
{
   std::cout << "ConnectionManager::stopAll" << std::endl;
...
{
   std::cout << "ConnectionManager::gracefullyStopAll" << std::endl;
   for (auto& connection : connections_) {
      connection->sendBye();
      connection->stop();
   }
